...
public class CprDecoder {
    ...
    public static GeoPos decodePosition(double x0, double y0, double x1, double y1, int mostRecent){
        ...
        if (Double.isNaN(a0)){
            ...
            lon0 = (1.0/lonZones0) * (lonZone0 + x0);
            lon1 = (1.0/lonZones1) * (lonZone1 + x1);
        }
        if (mostRecent == 0){
            if (lon0 > 0.5) lon0 -= 1;
            if (lat0 > 0.5) lat0 -= 1;
            int lonT32 = (int) Math.rint(Units.convert(lon0,Units.Angle.TURN,Units.Angle.T32));
            int latT32 = (int) Math.rint(Units.convert(lat0,Units.Angle.TURN,Units.Angle.T32));
            if (!GeoPos.isValidLatitudeT32(latT32)) return null;
            return new GeoPos(lonT32,latT32);
        }
        if (lon1 > 0.5) lon1 -= 1;
        if (lat1 > 0.5) lat1 -= 1;
        int lonT32 = (int) Math.rint(Units.convert(lon1,Units.Angle.TURN,Units.Angle.T32));
        int latT32 = (int) Math.rint(Units.convert(lat1,Units.Angle.TURN,Units.Angle.T32));
        if (!GeoPos.isValidLatitudeT32(latT32)) return null;
        return new GeoPos(lonT32,latT32);
    }
}
